  ...
  h.onMessage([&pid_steer, &pid_throttle, &new_delta_time, &timer, &prev_timer, &i, &prev_timer](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode)
  {
        ...
        if (s != "") {
          ...
          **/
          ...
          auto target_velocity = v_points.back();

		  // The throttle error will be computed by subtracting the target velocity from the current velocity
          // Since the total error using the negative of the coefficients; the negative result will increase the throttle 
          // i.e. if the car velocity is lower than the target velocity, the throttle should be possitive
